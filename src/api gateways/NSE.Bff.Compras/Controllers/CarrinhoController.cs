...
namespace NSE.Bff.Compras.Controllers
{
    ...
    public class CarrinhoController : MainController
    {
        ...
        {
            var voucher = await _pedidoService.ObterVoucherCodigo(voucherCodigo);
            if (voucher is null)
            {
        ...
        private async Task ValidarItemCarrinho(ItemProdutoDTO produto, int quantidade, bool adicionarProduto = false)
        {
            ...
            if (quantidade < 1) AdicionarErroProcessamento($"Escolha pelo menos 1 unidade do produto {produto.Nome}");

            var carrinho = await _carrinhoService.ObterCarrinho();
            var itemCarrinho = carrinho.Itens.FirstOrDefault(p => p.ProdutoId == produto.Id);

            if (itemCarrinho != null && adicionarProduto && itemCarrinho.Quantidade + quantidade > produto.QuantidadeDeStock)
            {
                AdicionarErroProcessamento($"O produto {produto.Nome} possui {produto.QuantidadeDeStock} unidades em stock, você selecionou {quantidade}");
                return;
            }

            if (quantidade > produto.QuantidadeDeStock)
                AdicionarErroProcessamento($"O produto {produto.Nome} possui {produto.QuantidadeDeStock} unidades em stock, você selecionou {quantidade}");
        }
    }
}