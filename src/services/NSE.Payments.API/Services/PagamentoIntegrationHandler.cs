...
namespace NSE.Payments.API.Services
{
    public class PagamentoIntegrationHandler : BackgroundService
    {
        ...
        private void SetSubscribers()
        {
            ...
            _bus.SubscribeAsync<PedidoBaixadoStockIntegrationEvent>("PedidoBaixadoStock", async request =>
        ...
        private async Task CapturarPagamento(PedidoBaixadoStockIntegrationEvent message)
        {
            using (var scope = _serviceProvider.CreateScope())
            {
                var pagamentoService = scope.ServiceProvider.GetRequiredService<IPagamentoService>();
